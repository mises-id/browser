    ...
    headerRight: (
      <TouchableOpacity
        ...
        <IonicIcon
          name={'ios-close'}
          size={38}
          style={[styles.backIcon, styles.backIconIOS]}
        />
      </TouchableOpacity>
    ),
  };
      ...
      transactionMode !== 'edit' ? (
        <TouchableOpacity
          ...
          <Text
            style={
              disableModeChange
                ? [styles.closeButtonText, styles.disabled]
 ...
 * Function that returns the navigation options
 * This is used by views that will show our custom navbar
 * which contains accounts icon, Title or Logo and current network, and settings icon
 *
 * @param {Object} navigation - Navigation object required to push new views
 ...
 * which is just the log and the Back button
 *
...
export function getOnboardingNavbarOptions(navigation, {headerLeft} = {}) {
  ...
  return {
    ...
    headerTitle: () => (
      <View style={styles.appNameTransparentWrapper}>
        <Image source={app_name} style={styles.appName} resizeMethod={'auto'} />
      </View>
    ),
...
export function getWalletNavbarOptions(title, navigation) {
  ...
  return {
    ...
    headerRight: () => (
      <TouchableOpacity style={styles.infoButton} onPress={openQRScanner}>
...
export function getSwapsQuotesNavbar(navigation) {
  const title = navigation.getParam('title', 'Swap');
  const leftActionText = navigation.getParam(
    'leftAction',
    strings('navigation.back'),
  );
